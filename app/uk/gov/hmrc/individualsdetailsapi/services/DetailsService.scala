...
import uk.gov.hmrc.individualsdetailsapi.domains.{Individual, MatchNotFoundException, MatchedCitizen, SandboxDetailsData}
import uk.gov.hmrc.individualsdetailsapi.domains.SandboxDetailsData._
import uk.gov.hmrc.individualsdetailsapi.domains.integrationframework.{IfContactDetail, IfDetailsResponse, IfResidence}
...
trait DetailsService {
  ...
  def getContactDetails(  matchId: UUID,
                       ...
                       ( implicit hc: HeaderCarrier,
                          ec: ExecutionContext ): Future[Seq[IfContactDetail]]

  def getResidences(  matchId: UUID,
                      endpoint: String,
                      scopes: Iterable[String] )
                   ( implicit hc: HeaderCarrier,
                     ec: ExecutionContext ): Future[Seq[IfResidence]]
}

@Singleton
class SandboxDetailsService  @Inject()( cacheService: CacheService,
  ...
  override def resolve(matchId: UUID)(implicit hc: HeaderCarrier): Future[MatchedCitizen] =
    if (matchId.equals(sandboxMatchId)) successful(MatchedCitizen(sandboxMatchId, sandboxNino))
    else failed(new MatchNotFoundException)

  override def getContactDetails( matchId: UUID,
                                  endpoint: String,
                                  scopes: Iterable[String])
                                ( implicit hc: HeaderCarrier,
                                  ec: ExecutionContext ): Future[Seq[IfContactDetail]] = ???

  override def getResidences( matchId: UUID,
                                  endpoint: String,
                                  scopes: Iterable[String])
                                ( implicit hc: HeaderCarrier,
                                  ec: ExecutionContext ): Future[Seq[IfResidence]] = ???
...
class LiveDetailsService @Inject()(
  ...
  private def getFromCacheOrIf[T]( matchId: UUID,
                            endpoint: String,
                            scopes: Iterable[String])
    ...
    cacheService.get(
      cacheId,
      {
        resolve(matchId).flatMap(ninoMatch => {
          ...
          ifConnector.fetchDetails(ninoMatch.nino, Option(fieldsQuery).filter(_.nonEmpty))
        })
      }) map responseMapper
  }
  ...
  override def getContactDetails( matchId: UUID,
    ...
    getFromCacheOrIf[Seq[IfContactDetail]](matchId, endpoint, scopes) {
      response => response.contactDetails.getOrElse(Seq())
    }
  }
  ...
  override def getResidences( matchId: UUID,
    ...
    getFromCacheOrIf[Seq[IfResidence]](matchId, endpoint, scopes) {
      response => response.residences.getOrElse(Seq())
    }
  }
}
