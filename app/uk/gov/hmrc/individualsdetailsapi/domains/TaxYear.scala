...
object TaxYear {
  ...
  private final def firstDayOfTaxYear(year: Int): LocalDate =
    new MonthDay(4, 6).toLocalDate(year)
  ...
  def fromEndYear(endYear: Int): TaxYear =
    TaxYear(s"%04d-%02d".format(endYear - 1, endYear % 100))

  def isValid(taxYearReference: String) =
    matchTaxYear(taxYearReference) exists { r =>
      (r.group("first").toInt + 1) % 100 == r.group("second").toInt
    }

  def current(): TaxYear = {
    val date =
      new LocalDate(DateTime.now(), DateTimeZone.forID("Europe/London"))
